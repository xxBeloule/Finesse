...
$regexText = '/^[a-zA-ZáàâäãåçéèêëíìîïñóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ.\s-]{3,100}$/';
...
if (count($_POST) > 3) {
    if (!empty($_POST['create'])) {
        ...
        if (!empty($title) && !empty($description) && !empty($image) && empty($error_title) && empty($error_size) && empty($error_type) && empty($error_description) && empty($error_price)) {
